...
bool MONTE_CARLO_SYSTEM::handleKeyPressedEvent( int key )
{
	...
	case 'm':
		...
		break;
	case ',':
		mNumSamples-=100/* = mMinNumSamples*/;
		flgHandled = true;
		break;
	case '.':
		mNumSamples+=100 /*= mMinNumSamples*/;
...
void MONTE_CARLO_SYSTEM::computeSamples( )
{
	...
	{
		mX[i] = (rand() / (double)RAND_MAX)*(mMaxX - mMinX) + mMinX;
		mY[i] = (rand() / (double)RAND_MAX)*(mMaxY - mMinY) + mMinY;
	}
}
...
double MONTE_CARLO_SYSTEM::computeArea( ) const
{
    ...
    */
	double area;
	double ratio;
	double all_area = (mMaxX - mMinX)*(mMaxY - mMinY);
	int inside_count=0;
	...
	ratio = ((double)(inside_count)) /((double) mNumSamples);
...
{
    double realY = getValue( x );//由X值帶出Y值，看樣本點的Y值就知道在不在函數圖形的區域內
	bool flag = false;
	if (realY > 0)
	{
		if (y > 0)
