...
void SIMPLE_FILE_EDITOR::readFile( )
{
	...
	ifstream fptr;
	fptr.open(FILE_NAME);
	/*while (fscanf(fptr, "%c", mBitmap) != EOF)   //cannot use fgets SINCE WE DO NOT WANT BEXT LINE CHARACTER but in this question we do not want
	next line character to be eaten in	
	{
		fgets(file_string, 41, fptr);
		string str(file_string);
		if (str.size() == 0)
			continue;
		storeOneRowToBitmap(str);
		cout << str << endl;
	}*/
	while (fptr)
	{
