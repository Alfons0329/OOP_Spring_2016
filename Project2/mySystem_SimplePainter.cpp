...
void SIMPLE_PAINTER::reset()
{
	...
	for (int i = 0; i < mNY*mNX; ++i) {
		...
		double r = (rand() / (double)(RAND_MAX))*(0.15)+0.75;
		double g = (rand() / (double)(RAND_MAX))*(0.15)+0.75;
		double b = (rand() / (double)(RAND_MAX))*(0.15)+0.75;
...
void SIMPLE_PAINTER::drawAt(int x, int y, const vector3 &color, double w)
{
	...
	{		
		vector3 cur_color = mCanvas[x + y*mNX];
		mCanvas[x+y*mNX] = cur_color*(1.0 - w*w) + w*w*color;
	}
}
